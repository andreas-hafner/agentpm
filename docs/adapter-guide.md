...
`<scope>/.kimi-code/skills/agent-<kebab-name>/SKILL.md` for each flat agent entry. Kimi Code
has no user-defined sub-agents, so the generated skill instructs Kimi's main agent to spawn
its built-in `Agent` tool with the role brief (the markdown body) as the prompt preamble.
Frontmatter `tools` decides the built-in profile: a read-only tool set (`Read`, `Grep`,
`Glob`) maps to `explore`, anything else (or no `tools` field) to `coder`. The generated
skill carries a `<!-- generated by agentpm from ... -->` marker; files without the marker
