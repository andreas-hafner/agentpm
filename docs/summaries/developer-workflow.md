...
- `scripts/prepare-readme-demo.mjs` resets the repo-local `.tmp/readme-demo` workspace used by `docs/assets/agentpm-demo.tape` so the README VHS walkthrough can render a deterministic install-and-push flow.
...
- GitHub Actions publishes only the `@travelhawk/agentpm` CLI package through `.github/workflows/publish-npm.yml`, and actual publishes are restricted to releases or manual runs from `master`. The npm token must be configured as the `NPM_TOKEN` repository secret, and internal `@agentpm/*` packages remain private while the CLI bundle carries their runtime code.
