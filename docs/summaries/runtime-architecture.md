...
- `agentpm push` normalizes pushed entries to a canonical `skills/<name>` folder by default; `--preserve-layout` keeps native target-relative paths.
...
- `agentpm adopt` (`AgentPmService.adopt`) moves an existing local skill into the library, replaces the origin with a managed symlink, and fans it out to the other agents the same way as pull. Both reuse `ensureManagedLink`, `recordGeneratedTargetInLocalGitExclude`, and a backing source row via `db.upsertSource` (installs require a non-null `source_id`).
