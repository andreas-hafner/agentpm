...
- `classifyLocator` determines source kind from a locator string. `isSkillsShLocator` recognizes skills.sh URLs, `registry:<url-or-path>` is classified as `'registry'`, and `file://` locators are treated as Git sources.
- `SourceKind` is `'git' | 'local' | 'registry'`. `skills.sh` is the only built-in API-backed registry; other registries use static index files under the same `registry` kind.
