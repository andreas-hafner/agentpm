  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const kimiSkillLink = path.join(fakeUserHome, '.kimi-code', 'skills', 'demo');
        expect((await fs.lstat(kimiSkillLink)).isSymbolicLink()).toBe(true);

        const kimiAgentSkill = await fs.readFile(
          path.join(fakeUserHome, '.kimi-code', 'skills', 'agent-reviewer', 'SKILL.md'),
          'utf8',
        );
        expect(kimiAgentSkill).toContain(
          '<!-- generated by agentpm from .claude/agents/reviewer.md - do not edit -->',
        );
        expect(kimiAgentSkill).toContain('name: agent-reviewer');
        expect(kimiAgentSkill).toContain('Review carefully.');
