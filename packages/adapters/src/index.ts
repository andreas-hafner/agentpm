...
import { isGeneratedKimiAgentSkill } from './transforms/kimi-agent.js';
...
async function detectGroupsForLayout(
  ...
  // Generated Kimi delegation skills (`.kimi-code/skills/agent-*`) can be
  // rediscovered by scan roots other than the dedicated `kimi` layout - e.g.
  // the catch-all "Deep skills" root, which walks the entire repo looking
  // for any `SKILL.md` - so the exclusion applies to every skill-kind scan,
  // not just `layout.adapter === 'kimi'`.
  const visibleEntries =
    layout.kind === 'skill'
      ? await excludeGeneratedKimiAgentSkills(entries, rootPath)
      : entries;
...
/**
 * Filter out `.kimi-code/skills/agent-*` entries generated by the
 * `kimi-agents` transform (marked with `isGeneratedKimiAgentSkill`) from
 * skill detection. Unlike the Codex transform's output (`.codex/agents`,
 * outside every scanned layout), Kimi delegation skills live inside the
 * same root as real installable Kimi skills, so `agentpm push` would
 * otherwise pick them up as user-authored skills and round-trip them into
 * the canonical library.
 */
async function excludeGeneratedKimiAgentSkills(
  entries: DetectedEntry[],
  rootPath: string,
): Promise<DetectedEntry[]> {
  const kept: DetectedEntry[] = [];
  for (const entry of entries) {
    const skillMdPath = path.join(rootPath, entry.relativePath, 'SKILL.md');
    const content = await fs.readFile(skillMdPath, 'utf8').catch(() => null);
    if (content !== null && isGeneratedKimiAgentSkill(content)) {
      continue;
    }
    kept.push(entry);
  }
